...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (chatbox && chatInput && userList) {
    ...
    socket.on("userLogout", (username) => {
      ...
      userList.innerHTML = userList.innerHTML.replace(
        `<li>${username}</li>`,
        ""
      );
    });
  ...
  if (allUsersList && searchInput) {
    // Check if you are on the userlist page
        ...
        users.forEach((username) => {
          ...
          button.addEventListener("click", async() => {
            ...
            const response = await fetch ('http://localhost:5000/chat/history', { 
              ...
              body: JSON.stringify( { username:currentUser, otherUsername:username } )
            });
            const data = await response.json();
            console.log(data);
            // Generate unique room ID
            // const roomId = uuidv4();
            // Emit startPrivateChat event
            socket.emit("startPrivateChat", {
              roomId: data.roomId,
              users: [sessionStorage.getItem("username"), username],
            });

            // Redirect user to private chat room
            window.location.href = `/app/privatechat?roomId=${data.roomId}`;
          });
    ...
    searchInput.addEventListener("keyup", (event) => {
      ...
      for (let i = 0; i < li.length; i++) {
        const username = li[i].firstChild.textContent; // get the username text node
